...
def game_loop(window, difficulty=None):
    ...
    fruits = [get_new_fruit(window=window) for _ in range(random.randint(1, 5))]
    score = 0
    ...
    while True:
        ...
        move_snake(snake, direction, fruits)

        if snake_hit_itself(snake) or actor_hit_border(actor=snake[0], window=window):
            break
        ...
        while len(fruits) < 5 and random.random() < 0.3:
            new = get_new_fruit(window)
            while new in fruits:
                new = get_new_fruit(window)
            fruits.append(new)
...
def move_snake(snake, direction, fruits):
    head = snake[0].copy()
    move_actor(head, direction)
    snake.insert(0, head)
    if head not in fruits:
        snake.pop()
...
def intro_animation(window):
    ...
    snake_frames = ["oOoOoO~", "~oOoOoO", "OoOoOo>", "<OoOoOo"]
    for _ in range(8):
